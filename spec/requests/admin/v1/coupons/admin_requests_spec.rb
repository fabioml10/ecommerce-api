...
RSpec.describe "Admin::V1::Coupons as :admin", type: :request do
  ...
  context "GET /coupons" do
    ...
    it "returns all Coupons" do
      ...
      expect(body_json['coupons']).to contain_exactly *coupons.as_json(only:  %i(id name code discount_value due_date max_use status))
    end
  ...
  context "POST /coupons" do
    ...
    context "with valid params" do
      ...
      it 'returns last added Coupon' do
        post url, headers: auth_header(user), params: coupon_params
        expected_coupon = Coupon.last.as_json(only: %i(id name code discount_value due_date max_use status))
  ...
  context "PUT /coupons/:id" do
    ...
    context "with valid params" do
      ...
      it 'returns updated Coupon' do
        patch url, headers: auth_header(user), params: coupon_params
        coupon.reload
        expected_coupon = coupon.as_json(only: %i(id name code discount_value due_date max_use status))
        expect(body_json['coupon']).to eq expected_coupon
      end
