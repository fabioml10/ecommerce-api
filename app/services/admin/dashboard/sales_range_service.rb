  ...
  class SalesRangeService
    ...
    def call
      if @max_date.present? && @min_date.present? && @max_date - @min_date < 1.month
        @records = format_records(group_sales_by_day)
      else
        @records = format_records(group_sales_by_month)
      end
    end
    ...
    def line_item_arel
      return @line_item_arel if @line_item_arel.present?
    ...
    def order_arel
      return @order_arel if @order_arel.present?
